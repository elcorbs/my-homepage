...
export async function getRecipes(callback) {
  const schema = `query={recipes{name, cuisine, ingredients{name, amount, measurement}, method}}`;
  await queryApi(schema, data => callback(data.recipes))
}

export async function getRecipe(name, callback){
  const schema = `query={recipe(name: "${name}"){name, cuisine, ingredients{name, amount, measurement}, method}}`;
